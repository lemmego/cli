...
function Install-Lemmego {
    ...
    if ($arch -match "64-bit") {
        if ($env:PROCESSOR_ARCHITEW6432 -eq "ARM64") {
            $downloadUrl = "https://github.com/lemmego/cli/releases/download/v0.1.32/lemmego-v0.1.32-windows-arm64.exe"
        } else {
            $downloadUrl = "https://github.com/lemmego/cli/releases/download/v0.1.32/lemmego-v0.1.32-windows-amd64.exe"
        }
    } else {
