...
download_binary() {
    ...
    case $OS in
        darwin|linux)
            if [ "$ARCH" = "amd64" ]; then
                download_url="https://github.com/lemmego/cli/releases/download/v0.1.29/lemmego-v0.1.29-$OS-amd64"
            elif [ "$ARCH" = "arm64" ]; then
                download_url="https://github.com/lemmego/cli/releases/download/v0.1.29/lemmego-v0.1.29-$OS-arm64"
            fi
            ;;
        *)
    ...
    curl -L "$download_url" -o "lemmego-v0.1.29-$OS-$ARCH"
    if [ $? -ne 0 ]; then
    ...
    sudo mv "lemmego-v0.1.29-$OS-$ARCH" /usr/local/bin/lemmego
    if [ $? -ne 0 ]; then
